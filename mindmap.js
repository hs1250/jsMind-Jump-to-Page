...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- NEW: Quick Jump Element References ---
  const slideJumpContainer = document.getElementById('slide_jump_container');
  const slideCounterText = slideJumpContainer.querySelector('.slide-counter-text');
  const slideJumpInput = document.getElementById('slide_jump_input');
  ...
  const showPresentationSlide = (index, direction = 'none') => {
    ...
    // --- UPDATED: Quick Jump ---
    const counterText = overlay.querySelector('.slide-counter-text');
    const counterInput = overlay.querySelector('#slide_jump_input');
    // --- END UPDATED ---
    ...
    // --- UPDATED: Update counter text and hide input ---
    counterText.textContent = `${index + 1} / ${presentationSlides.length}`;
    if (counterInput) {
        counterInput.max = presentationSlides.length;
        counterInput.style.display = 'none'; // 確保輸入框在換頁時隱藏
    }
    counterText.style.display = 'inline-block'; // 確保文字顯示
    // --- END UPDATED ---
...
const showPresentationSlide=(index,direction='none')=>{
  ...
  counterText.textContent=(index+1)+' / '+presentationSlides.length;
  if(counterInput){counterInput.max=presentationSlides.length;counterInput.style.display='none';}
...
const slideCounterText=slideJumpContainer.querySelector('.slide-counter-text');
const slideJumpInput=document.getElementById('slide_jump_input');
slideCounterText.addEventListener('click',(e)=>{
  e.stopPropagation();
  if(!isInPresentationMode)return;
  slideCounterText.style.display='none';
  slideJumpInput.style.display='inline-block';
  slideJumpInput.value=currentSlideIndex+1;
  slideJumpInput.select();
});
slideJumpInput.addEventListener('keydown',(e)=>{
  if(e.key==='Enter'){
    e.preventDefault();
    const newSlideNum=parseInt(slideJumpInput.value,10);
    if(newSlideNum&&newSlideNum>=1&&newSlideNum<=presentationSlides.length){
      showPresentationSlide(newSlideNum-1,'slide');
    }else{
      slideJumpInput.style.display='none';
      slideCounterText.style.display='inline-block';
    }
...
slideJumpInput.addEventListener('blur',()=>{
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // --- UPDATED: Stop keyboard navigation if input is active ---
    if (document.activeElement === slideJumpInput) {
        return;
    }
    // --- END UPDATED ---
  ...
  slideCounterText.addEventListener('click', (e) => {
    ...
    slideCounterText.style.display = 'none';
    slideJumpInput.style.display = 'inline-block';
    slideJumpInput.value = currentSlideIndex + 1; // 顯示當前頁碼 (1-based)
    slideJumpInput.select(); // 自動全選文字
  });

  // 2. 在輸入框中按下 Enter 或 Escape
  slideJumpInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const newSlideNum = parseInt(slideJumpInput.value, 10);
      ...
      if (newSlideNum && newSlideNum >= 1 && newSlideNum <= presentationSlides.length) {
        // 合法的頁碼，跳轉 (將 1-based 轉為 0-based index)
        showPresentationSlide(newSlideNum - 1, 'slide'); 
      } else {
        // 不合法的數字，恢復原狀
        slideJumpInput.style.display = 'none';
        slideCounterText.style.display = 'inline-block';
        showNotification('請輸入 1 到 ' + presentationSlides.length + ' 之間的數字', true);
      }
    } else if (e.key === 'Escape') {
      // 按下 Escape 取消
      e.preventDefault();
  ...
  // 3. 當輸入框失去焦點時 (點擊旁邊)，取消輸入
  slideJumpInput.addEventListener('blur', () => {
    slideJumpInput.style.display = 'none';
    slideCounterText.style.display = 'inline-block';
  });
  // --- END: Quick Jump Logic ---
